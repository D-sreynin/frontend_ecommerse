...
const Size = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    http.put(`/sizes/${selectedSize.id}`, formData)
      .then(response => {
        setSizes(sizes.map(product => 
      ...
      <div className="page-inner">
        <div className="d-flex align-items-left align-items-md-center flex-column flex-md-row pt-2 pb-4">
          ...
          <div className="ms-md-auto py-2 py-md-0">
            ...
            <a href="/createsize" className="btn btn-primary btn-round">Add Size</a>
          </div>
        </div>
        <div>
        <div>
      <h1 className="text-2xl font-semibold mb-4" style={{textAlign:"center"}}>Size List</h1>
      <div className='tbrnt'>
        <table className="min-w-full table-auto bg-white border border-gray-200 rounded-lg shadow-md" style={{width:"100%"}}>
          <thead>
            <tr className="bg-gray-100 text-center text-sm font-medium text-gray-700" >
              ...
              <th className="px-4 py-3 border-b"style={{fontSize:"20px"}}>Weight</th>
            </tr>
          </thead>
          <tbody>
          {sizes.map((size) => (
            <tr key={size.id} style={{textAlign:"center"}}>
                ...
                <button onClick={() => handleEdit(size)} className="text-blue-500 hover:text-blue-700" style={{backgroundColor:"blue", color:"white", borderRadius:"7px", border:"none"}}>Edit</button>
                <button onClick={() => handleDelete(size.id)} className="text-red-500 hover:text-red-700 ml-2" style={{backgroundColor:"red", color:"white", borderRadius:"7px"}}>Delete</button>
              </td>
              <td className="px-4 py-3 border-b">{size.name}</td>
              <td className="px-4 py-3 border-b">{size.description} kg</td>
            </tr>
          ))}
          </tbody>
        </table>
